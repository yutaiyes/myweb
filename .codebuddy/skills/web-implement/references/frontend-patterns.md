...
All page components should use framer-motion to add micro-animations for a polished feel.
...
{/* With custom delay */}
<FadeIn delay={0.2} variants={scaleUp}>
  <PricingCard />
</FadeIn>
```
...
**Typical page composition with MotionPrimitives:**
```tsx
    ...
    <div className="space-y-20">
      ...
      <FadeIn variants={fadeDown}>
        <section className="text-center py-20">
          ...
          <p className="text-xl text-muted-foreground mt-4">Subtitle here</p>
        </section>
      </FadeIn>
...
- Always import from `framer-motion` or `@/components/MotionPrimitives` — do NOT use manual CSS transitions for page/component animations.
- Keep durations short: 0.2–0.5s for UI interactions, 0.3–0.6s for page transitions.
- Use `type: "spring"` for interactive elements (buttons, cards).
- Use `whileInView` with `viewport={{ once: true }}` for scroll animations.
- Use `staggerChildren` (or `<Stagger>`) for lists/grids.
