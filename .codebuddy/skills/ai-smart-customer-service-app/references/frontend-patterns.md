...
- **Always prefer `<FadeIn>` / `<Stagger>` / `<HoverLift>`** over raw `motion.div` for standard patterns.
- Always import from `framer-motion` or `@/components/MotionPrimitives` — do NOT use CSS transitions for page/component animations.
- **Keep durations short**: 0.2–0.5s for UI interactions, 0.3–0.6s for page transitions.
- **Use `type: "spring"` for interactive elements** (buttons, cards) — feels more natural.
- **Use `ease: "easeOut"` for entrance animations** — fast start, gentle end.
- **Use `whileInView` with `viewport={{ once: true }}`** (or `<FadeIn>`) for scroll animations — prevents re-triggering.
- **Use `staggerChildren`** (or `<Stagger>`) for lists/grids — adds visual rhythm.
- **Do NOT animate layout-breaking properties** (width/height of text containers) unless using `layout` prop.
